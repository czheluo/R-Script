...
c1<-read.table("new_cluster_2",header=T,row.names=1,sep="\t")
c1<-read.table("new_cluster_3",header=T,row.names=1,sep="\t")
#x<-as.matrix(t(c1))
res.fcm <- fcm(c1, centers=5)
membership<-res.fcm$u[,1]
mem=as.matrix(as.numeric(membership))
colnames(mem)="mem"
c=cbind(c1,mem)
c1=c
c1=c1[order(c1[,'mem']),]
...
pdf("cluster_3.pdf",width=10,height=10)
...
axis(1, at=1:5, lab=c("D1","W2","W4","W8","W12"))
...
png("cluster_3.png",width=1000,height=800)
par(mar=c(4.5, 4.5, 1, 6))
plot(c(1:5),c1[1,c(1:5)],xlim=c(0.5,5), ylim=c(-2,1),type="l",col=rainbow(105)[1],axes=FALSE, ann=FALSE)
for (i in 2:105){# Draw first line
	lines(c(1:5),c1[i,c(1:5)], type = "l", col = rainbow(105)[i])                   # Add second line
